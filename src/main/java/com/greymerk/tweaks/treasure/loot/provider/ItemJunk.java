...
public class ItemJunk extends ItemBase{
	...
	public ItemStack getLootItem(Random rand, int level){
		...
		if(level > 0 && rand.nextInt(200) == 0){
			if(level > 2 && rand.nextInt(10) == 0) return new ItemStack(Items.DIAMOND_HORSE_ARMOR, 1);
			if(level > 1 && rand.nextInt(5) == 0) return new ItemStack(Items.GOLDEN_HORSE_ARMOR, 1);
			if(rand.nextInt(3) == 0) return new ItemStack(Items.IRON_HORSE_ARMOR, 1);
		...
		if(level > 1 && rand.nextInt(50) == 0){			
			...
			case 5: return new ItemStack(Items.LAPIS_LAZULI, 1 + rand.nextInt(3));
			}
		}
